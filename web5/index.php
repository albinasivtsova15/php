...
$user = 'u52883';
$pass = '4166899';
$db = new PDO('mysql:host=localhost;dbname=u52883', $user, $pass, array(PDO::ATTR_PERSISTENT => true));
...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
  ...
  if (!empty($_COOKIE['save'])) {
    ...
    if (!empty($_COOKIE['password'])) {
      $messages['login'] = sprintf('<div class="login">Логин: <strong>%s</strong><br>
        Пароль: <strong>%s</strong><br><a href=login.php>Войдите</a> в аккаунт с этими данными,<br>чтобы изменить введёные значения формы</div>',
  ...
  if (empty($limbs)) {
    ...
    $errors = TRUE;
  } else if ($limbs != '2' && $limbs != '4' && $limbs != '6') {
  ...
  if (!empty($_COOKIE[session_name()]) && session_start() && !empty($_SESSION['login'])) {
    if (!empty($_POST['token']) && hash_equals($_POST['token'], $_SESSION['token'])) {
      ...
      try {
        ...
        if (array_diff_assoc($abil, $abilities)) {
